...
//    double mchi=2.0*nuobs*2.0*Pi*6.5822e-16; // GeV, if decay into two photons
    ...
    idSphrec=idSph;
    rintin=1.e-3;
    rintfin=0.2; // setting 200 pc for LeoT // max(5.*r0[idSphrec+numdSph*DMprofdSph[idSphrec]],dist*sin(30.0*60.*aminrad));  // setting rfin=3*r0DM
...
/* routine to load one dSph image from a 3D cube*/
   int nmapsinput=1;
   if(userms) {nmapsinput=2;
        if(useflag) {nmapsinput=3;
            if(usemask) nmapsinput=4;}} // number of maps/cubes:1=only data; 2=data+err; 3=data+err+flags; 4=data+err+flags+maskcube
...
//    ra.clear(); dec.clear();
     ...
     for (int imap=0;imap<nmapsinput;imap++){
       ...
       if(imap<3) filevar.append(mapname);
       if(imap==3) filevar.append(maskname); // used only if loop has 4 iterations (to be set above)
       cout<<"filevar = "<<filevar<<endl;

       
       
       const char *filename; filename=filevar.c_str();
       if( fits_open_image(&fptr,filename,READONLY,&status) ) cout<<"read observational map: open status= "<<status<<endl;
       fits_get_num_hdus(fptr,&hdunum,&status); // get hdu of last table
       
       if(imap==1) {fits_movabs_hdu(fptr,hdunum-1,&hdutype,&status);cout<<"rms. Moving to header number "<<hdunum-1<<" "<<filename<<endl;} // move to table before last      
       if(imap==2) {fits_movabs_hdu(fptr,hdunum,&hdutype,&status); cout<<"flag. Moving to header number "<<hdunum<<" "<<filename<<endl;} // move to last table

       if(imap==3) {fits_movabs_hdu(fptr,hdunum,&hdutype,&status); cout<<"mask. Moving to header number "<<hdunum<<" "<<filename<<endl;}// move to last table
...
//       cout<<imap<<"hdulast "<<hdunum<<" "<<hdutype<<endl; //fits_get_hdu_num(fptr,&hdunum); cout<<imap<<"hducurr "<<hdunum<<" "<<hdutype<<endl;
       ...
       if(imap<3) if( fits_read_key(fptr,TINT,"CRPIX1",&CRPIX1,comment,&status) ) cout<<"CRPIX1 = "<<CRPIX1<<" status= "<<status<<endl;
       if(imap<3) if( fits_read_key(fptr,TINT,"CRPIX2",&CRPIX2,comment,&status) ) cout<<"CRPIX2 = "<<CRPIX2<<" status= "<<status<<endl;
       if(imap<3) if( fits_read_key(fptr,TFLOAT,"CRVAL1",&CRVAL1,comment,&status) ) cout<<"CRVAL1 = "<<CRVAL1<<" status= "<<status<<endl;
       if(imap<3) if( fits_read_key(fptr,TFLOAT,"CRVAL2",&CRVAL2,comment,&status) ) cout<<"CRVAL2 = "<<CRVAL2<<" status= "<<status<<endl;
...
//       dec0=CRVAL2; // central position DEC in degree 
       ...
       if(imap==3) if( fits_read_img(fptr,TLONGLONG,felement,nelements,&nulval,map_insou,&anynul,&status) ) cout<<"read table status= "<<status<<endl;
       if(imap==2) if( fits_read_img(fptr,TUSHORT,felement,nelements,&nulval,map_in2,&anynul,&status) ) cout<<"read table status= "<<status<<endl;
       if(imap<2) if( fits_read_img(fptr,TFLOAT,felement,nelements,&nulval,map_in,&anynul,&status) ) cout<<"read table status= "<<status<<endl;
       fits_close_file(fptr, &status);    // close the file */
       
       if(imap==3) cout<<map_insou[322*100+100]<<endl;
       if(imap==2) cout<<map_in2[322*100+100]<<endl;
       if(imap<2) cout<<map_in[322*100+100]<<endl;
...
// instead of in a spherical space (the sky)
       for(int idec=0; idec<NAXIS2; idec++){
         if(imap==0&&ichannel==ichlow) dec.push_back(CDELT2*((CRPIX2-1)-idec));  // degree 
// above is actually m not dec // m=(sin(dec)*cos(dec0)-cos(dec)*sin(dec0)*cos(ra-ra0)))] however m-|dec-dec0|< arcsec in our maps (and we are not interested in DEC)
         for(int ira=0; ira<NAXIS1; ira++){
           if(idec==0&&imap==0&&ichannel==ichlow) ra.push_back(CDELT1*(ira-(CRPIX1-1))); // degree
...
//             flux=(20.0*1.116e57*1.6022e-3/pow(3.0856e21,2)*1.e20*0.762*1.e-25/deltanuobs/(4.0*Pi)/pow(maxdist/(beamx),2))/(binsize*1.0);// ERASE JUST FOR TESTING!!!!!!!!!!!!
             if(ichannel==ichlow) {map.push_back(flux); mapmodel.push_back(0.); maprms.push_back(rmsave); mapflag.push_back(0); mapsource.push_back(0.0); }
...
//         if(map[map.size()-1]>1.e-5) cout<< "Map "<<idec<<" "<<ira<<" "<<map_in[map.size()-1]<<endl;
           if(imap==2) {mapflag[idec*NAXIS1+ira]+=map_in2[idec*NAXIS1+ira]; 
   //          if(map_in2[idec*NAXIS1+ira]>0) cout<<"flag "<<map_in2[idec*NAXIS1+ira]<<" "<<mapflag[idec*NAXIS1+ira]<<endl;
           }
           if(imap==3) {mapsource[idec*NAXIS1+ira]+=map_insou[idec*NAXIS1+ira]; 
...
//         string filemask=data_path+"images/Leo_sextractor_fin.fits"; const char *filenamemask; filenamemask=filemask.c_str();
         string filemask=data_path+"images/Leo_sextractor_wpv_3s.fits"; const char *filenamemask; filenamemask=filemask.c_str();
         fitsfile *fptrmask;
...
//    cout<<mapflag[in]<<" ";
    if(idec<njump||ira<njump||idec>dec.size()-njump||ira>ra.size()-njump||abs(mapsource[in])>1.e-50||distvar>maxdist||mapflag[idec*ra.size()+ira]>=1) map[in]=0.0; }
//    if(idec<njump||ira<njump||idec>dec.size()-njump||ira>ra.size()-njump||distvar>maxdist||mapflag[idec*ra.size()+ira]>=1) map[in]=0.0; }

// Alternative derivation of the noise - Standard deviation computed in 20x20 boxes (or modify for different size)//
...
//    cout<< "Map "<<(ichlow+ichsup)/2.0<<" "<<ncountpix<<" "<<avespect<<" "<<sqrt(avestd/avevar)<<endl;
    double aveave=avespect/(ncountpix*1.0); // avespectw/avevarinv; // 
    double aveerr=sqrt(avevar)/(ncountpix*1.0); // 1.0/sqrt(avevarinv); //   sqrt(avestd)/(ncountpix*1.0);  // 
    ...
    outfile.close();
   egb=avespectw/avevarinv; //avespect/(ncountpix*1.0); // Comment this line to use weighted average as initial guess for Sflat. used as a first guess for egb
...
//      cout<<exp(tabCRx[ith])<<" "<<tabCRy[ith]*1.e3<<endl;//arcmin vs mJy
  ...
  for(int ith=0; ith<ntheta+1;ith++) tabCRy[ith]=log(max(tabCRy[ith],1.e-200)); 
  return; 
};
...
/* diffuse intensity I along a given los specified by thin in units of Jy or similar (having defined I=int dl j)
 ...
 return parrec[0]/(4.0*Pi*deltanuobs)*1.116e57*1.6022e-3/pow(3.0856e21,2)*1.e20*1.048e5*1.782*pow(thin/aminrad/60.0,1.782)/(2.0*Pi*thin*thin)*4*Pi; //the factor 4*Pi cancels with the one in diffint

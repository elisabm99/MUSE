...
//    ra.clear(); dec.clear();
     ...
     for (int imap=0; imap<nmapsinput;imap++){
       cout <<endl<< "Loading map number  = "<< imap+1 <<endl;
       ...
       if( fits_open_image(&fptr,filename,READONLY,&status) ) cout<<"read observational map: open status= "<<status<<" filevar= "<<filevar<<endl;
...
/*       if( fits_read_key(fptr,TFLOAT,"CDELT2",&CDELT2,comment,&status) ) cout<<"CDELT2 = "<<CDELT2<<" status= "<<status<<endl; */ CDELT2=0.2/3600.0;
       ...
       if(maptype<2) if( fits_read_img(fptr,TFLOAT,felement,nelements,&nulval,map_in,&anynul,&status) ) cout<<"***** read table status= "<<status<<endl;
       if(maptype==3) if( fits_read_img(fptr,TFLOAT,1,nelements,&nulval,map_in,&anynul,&status) ) cout<<"***** read table status= "<<status<<endl;
...
//             if(map_insou[idec*NAXIS1+ira]>0) cout<<"mask "<<map_insou[idec*NAXIS1+ira]<<" "<<mapsource[idec*NAXIS1+ira]<<endl;
       ...
       // Insert our mask made with sextracotr
...
//  abort();
  ...
  for(int idec=0; idec<dec.size(); idec++)  for(int ira=0; ira<ra.size(); ira++) { 
    ...
    if(idec<njump||ira<njump||idec>dec.size()-njump||ira>ra.size()-njump||abs(mapsource[in])>1.e-50||distvar>maxdist||mapflag[in]>=1 ||(useexp==1&&mapexp[in]<=0.2))   map[in]=0.0;
//    if (in==0)cout<<"distvar="<<distvar<<" maxdist="<<maxdist<<" abs(mapsource[in])="<<abs(mapsource[in])<<" mapflag[in]="<<mapflag[in]<<" mapexp[in]="<<mapexp[in]<<endl;
    else if(mapname == "DATACUBE_Eridanus2_field2.fits" && dec[idec]<=-0.006833333333333334 && ra[ira]>=0.00044444444444444447){
//        cout<<"Setting map to zero for pixel "<<ira<<" "<<idec<<" "<<in<<endl;
//        cout<<ra[ira]<<" "<<dec[idec]<<endl<<endl;
        map[in]=0.0;
    }
  }
...
// cout<< "Map "<<in<<" "<<ncountpix<<" "<<map[in]<<" "<<maprms[in]<<endl;}
  ...
  cout<<"\nMap "<<(ichlow+ichsup)/2.0<<" "<<ncountpix<<" "<<avespect<<" "<<sqrt(avestd/avevar)<<endl;
      ...
      if(idec%npixy==0||waydof!=2){
        for(int ira=0; ira<ra.size(); ira++){
          if(ira%npixx==0||waydof!=2){
            ...
            else{thetavar=sqrt(ra[ira]*ra[ira] + dec[idec]*dec[idec])*60.;} // arcmin
